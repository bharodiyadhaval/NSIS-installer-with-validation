...
bool SceneDescriptor(std::string file)
{
	...
	int count;
	plyFile >> count;
	ReadFileToToken(plyFile, "camoff");
	plyFile >> camoff;
...
static size_t WritetoServer(void *contents, size_t size, size_t member, void *user)
{
	((std::string*)user)->append((char*)contents, size * member);
	return size * member;
}

std::string user;
std::string reqpass;
std::string textlicense;
std::string serverReturn = "";
...
int main(void)
{
	...
	std::cout << "\n**************************************************************************" << std::endl;
	std::cout << "		AUTHENTICATION of Productkey					" << std::endl;
	std::cout << "****************************************************************************" << std::endl;
	...
	if (file.is_open())
	{
		while (file >> textlicense)
		{
				//std::cout << textlicense << std::endl;
		}
		file.close();
	}
	else
	...
	std::string post = "email=" + user + "&pass=" + reqpass;
	std::string temp;
	const char* Char = post.c_str();
	if (curl)
	{
		...
		curl_easy_setopt(curl, CURLOPT_URL, "http://localhost/cnd/UserValidation.php");
		/* Now specify the POST data */
		curl_easy_setopt(curl, CURLOPT_POSTFIELDS, Char);
		/* Perform the request, res will get the return code */
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, WritetoServer);
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, &serverReturn);
	...
	{
		std::cout << "Check your connection" << std::endl;
	}

	//std::cout << serverReturn.c_str() << std::endl;
	//std::cout << textlicense.c_str() << std::endl;
	textlicense = " " + textlicense;
	if (strcmp(serverReturn.c_str(), textlicense.c_str()) == 1)
	{
		std::cout << "Your license is not valid!!!!";
		system("pause");
		return 0;
	}
	...
	{
		if (my[i].type == PLANE && i != 0 && i != my.size() - 1 && i != my.size() - 2 && i != my.size() - 3)
		{

			g_vecGameObjects[i]->scale = my[0].scale;
			my[i].scale = my[0].scale;
			g_vecGameObjects[i]->position *= my[0].scale*glm::vec3(1, 1.1, 1);
			my[i].pos *= g_vecGameObjects[i]->position;
			g_vecGameObjects[0]->diffuseColour = glm::vec4(1, 1, 1, 0.1f);
		}
	}

	g_vecGameObjects[my.size() - 3]->scale = 1.5;
	g_vecGameObjects[my.size() - 3]->position = glm::vec3(-11.25 * my[0].scale - 10, 15, -13 * my[0].scale - 10);

	g_vecGameObjects[my.size() - 2]->scale = 1.5;
	g_vecGameObjects[my.size() - 2]->position = glm::vec3(15 * my[0].scale, 15, -13 * my[0].scale);

	g_vecGameObjects[my.size() - 1]->scale = 1.5;
	g_vecGameObjects[my.size() - 1]->position = glm::vec3(15 * my[0].scale, 15, 15 * my[0].scale);
	...
	LoadPly("Teapot", "Utah_Teapot_xyz.ply");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < ::g_vecGameObjects.size(); i++)
		{
			if (collision[i] == 1)
			{
				::g_vecGameObjects[i]->position = glm::vec3(13.5*my[0].scale - 5, 0, -13 * my[0].scale);
				my[i].vel = glm::vec3(0, 0, 0);
				disp[i] = glm::vec3(0, 0, 0);
				my[i].pos = ::g_vecGameObjects[i]->position;
				collision[i] = 0;
			}
			if (collision[i] == 2)
			{
				::g_vecGameObjects[i]->position = glm::vec3(-13.5*my[0].scale + 20, 0, -13 * my[0].scale);
				my[i].vel = glm::vec3(0, 0, 0);
				disp[i] = glm::vec3(0, 0, 0);
				my[i].pos = ::g_vecGameObjects[i]->position;
				collision[i] = 0;
			...
			if (collision[i] == 3)
			{
				::g_vecGameObjects[i]->position = glm::vec3(0, 0, 0);
				my[i].vel = glm::vec3(0, 0, 0);
				disp[i] = glm::vec3(0, 0, 0);
				my[i].pos = ::g_vecGameObjects[i]->position;
				collision[i] = 0;
			...
			//::g_vecGameObjects[i]->diffuseColour = glm::vec4(1.0,1.0,1.0,1.0);
			::g_vecGameObjects[i]->diffuseColour = my[i].color;
		}
